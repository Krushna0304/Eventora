...
```powershell
docker build -t eventora-ml-service .
```

Run the service in a container (expose port 5000):
```powershell
docker run -p 5000:5000 eventora-ml-service
```

This will start the ML service in a portable container. Adjust the port mapping if your service uses a different port or protocol.

You can pass environment variables or mount volumes as needed for production use.
